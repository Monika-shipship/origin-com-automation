...
9. Verify only result-defining invariants: record count/values, required column labels, formula/operation state, plot X/Y/label sources, axis state, and non-empty artifacts. Require `label_source_status=resolved` when labels are requested. For `origin_run_task`, inspect `methods.formulas` and `methods.analyses`: a successful task must report `metadata_verified=true` and `value_readback_verified=true` for formulas, and `resolved_backend="origin_native"`, `native_operation_created=true`, `editable_in_origin=true`, and a stable `operation_ref` for native analyses. Treat `ORIGIN_FORMULA_UNCONFIRMED` and `NATIVE_ANALYSIS_UNCONFIRMED` as hard failures; do not present static values as editable Origin results.
